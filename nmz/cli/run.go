...
import (
	...
	"github.com/ampf11111/namazu/nmz/explorepolicy"
	"github.com/ampf11111/namazu/nmz/historystorage"
	"github.com/ampf11111/namazu/nmz/orchestrator"
	"github.com/ampf11111/namazu/nmz/util/cmd"
	"github.com/ampf11111/namazu/nmz/util/config"
...
func (this *runner) initGlobalLogger() error {
	logutil.InitLog(path.Join(this.workingDirPath, "nmz.log"), logutil.Debug)
	return nil
}
...
Typical usage:
     $ nmz init --force config.toml materials /tmp/x
     $ for f in $(seq 1 100);do nmz run /tmp/x; done
     $ nmz tools summary /tmp/x

You have to prepare config.toml and the materials directory before running the init command.
Please also refer to the examples included in the github repository: https://github.com/ampf11111/namazu/tree/master/example

NOTE: "nmz run" is different from "nmz container run".
`
	return s
}

func (cmd runCmd) Run(args []string) int {
	log.Warn("`nmz run` is different from `nmz container run` (Docker-like CLI).")
	return run(args)
}
